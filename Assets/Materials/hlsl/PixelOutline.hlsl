...
void PixelOutline_float(UnityTexture2D tex, float2 uv, float4 outlineColor, out float4 color, out float alpha, out float hasOutline)
{
	...
	float left = SAMPLE_TEXTURE2D(tex, tex.samplerstate, uv - float2(texelSize.x, 0)).a;
	...
	float outline = saturate((left - color.a) + (right - color.a) + (top - color.a) + (bottom - color.a));
